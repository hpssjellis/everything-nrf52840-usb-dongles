...
int myHexToInt(const unsigned char data[], int length) {
  for (int i = 0; i < length; i++) {
    unsigned char b = data[i];
    if (b < 16) {
      return 0;
...
void myPrintHex(const unsigned char data[], int length) {
  Serial.print("0x");
  for (int i = 0; i < length; i++) {
    unsigned char b = data[i];
    if (b < 16) {
...
void controlLed(BLEDevice peripheral) {
  ...
  if (ledCharacteristic.canRead()) {
    ...
    Serial.print("ledCharacteristic.descriptor(0).uuid(): ");
    Serial.println((int)ledCharacteristic.descriptor(0).uuid());
